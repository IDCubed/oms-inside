# The Tools Used in `oms-happathon`

## Introduction

`oms-happathon` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `oms-happathon` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`oms-happathon` uses git to manage its codebase. While in theory you don't have
to use Git once you download `oms-happathon`, this project makes the assumption
...
While `oms-happathon` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `oms-happathon` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `oms-happathon`; Grunt is
...
to get started with `oms-happathon`. But for those curious or looking to go a
...
Grunt is the engine behind `oms-happathon`. It's the magic that makes it move.
...
`oms-happathon` comes with a `bower.json` file that looks something like this:
...
in `oms-happathon` by default in there to see what I mean.
